function renderHeader() {
    ...
    const initial = state.user ? state.user.name.charAt(0) : 'A';
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-3 pl-4 border-l border-[#334155]">
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-bold text-[#F8FAFC] leading-none">${state.user ? state.user.name : 'Administrador'}</p>
                        <p class="text-xs text-[#CBD5E1] font-medium mt-1">${state.user ? state.user.role : 'Super Admin'}</p>
                    </div>
