async function renderAdmin(c) {
    ...
    const rows = users.map(u => `
        <tr class="hover:bg-[#111827]/40 transition-colors border-b border-[#334155] last:border-0">
            <td class="p-4 whitespace-nowrap text-[#CBD5E1] font-mono text-xs">${u.id}</td>
            <td class="p-4 whitespace-nowrap text-[#F8FAFC] font-semibold">${u.name}</td>
            <td class="p-4 whitespace-nowrap text-[#CBD5E1]">${u.username}</td>
            <td class="p-4 whitespace-nowrap">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${u.role==='Super Admin' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-sky-500/20 text-sky-400 border border-sky-500/30'}">
                    ${u.role}
                </span>
            </td>
            <td class="p-4 whitespace-nowrap">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${u.status==='Activo' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}">
                    <span class="w-1.5 h-1.5 rounded-full ${u.status==='Activo'?'bg-emerald-500':'bg-red-500'}"></span>
                    ${u.status || 'Activo'}
                </span>
            </td>
            <td class="p-4 whitespace-nowrap text-right">
                <button class="p-2 text-[#CBD5E1] hover:text-[#F8FAFC] hover:bg-[#334155] rounded-lg transition-colors" onclick="openUserModal(${u.id})">
...
window.openUserModal = (id = null) => {
    const u = id ? MOCK_DB.users.find(x => x.id === id) : { username: '', password: '', role: 'Vendedor', name: '', status: 'Activo' };
    ...
    const html = `
        <div class="p-6 text-slate-800">
            ...
            <form id="user-form" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Nombre Real</label>
                    <input type="text" id="u-name" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:border-blue-500 focus:ring-blue-500 py-2.5" value="${u.name}" required placeholder="Ej. Juan Pérez">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="u-status" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:border-blue-500 focus:ring-blue-500 py-2.5">
                            <option value="Activo" ${u.status!=='Inactivo'?'selected':''}>Activo</option>
    ...
    document.getElementById('user-form').onsubmit = (ev) => {
        ...
        if(uid) {
            const index = MOCK_DB.users.findIndex(x => x.id == uid);
            if(index > -1) {
                MOCK_DB.users[index] = { ...MOCK_DB.users[index], ...userData };
                if(pass) MOCK_DB.users[index].password = pass;
            }
        } else {
            userData.id = MOCK_DB.users.length ? Math.max(...MOCK_DB.users.map(x=>x.id))+1 : 1;
            userData.password = pass;
            MOCK_DB.users.push(userData);
        }
