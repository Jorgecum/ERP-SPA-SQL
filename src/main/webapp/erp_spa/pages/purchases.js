...
window.openCompraModal = () => {
    ...
    window.linkOCModal = (orders, prods) => {
        const ocId = document.getElementById('c-order-link').value;
        if (!ocId) return;

        const ocCompleta = await api.getPurchaseOrderById ? await api.getPurchaseOrderById(ocId) : orders.find(o => String(o.id) === String(ocId));
        if (!ocCompleta) return;

        const providerId = getOrderProviderId(oc);
        ...
        window.compraModalState.tempItems = getOrderItems(oc).map(i => {
